...
```bash
# 启用调试日志
drun run test_api -env dev -log-level DEBUG -secrets plain
```
...
```bash
# 调试查看响应
drun run test_api -env dev -log-level DEBUG
```
...
**解决**：使用 `-secrets mask`
```bash
drun run testcases -env dev -html report.html -secrets mask
```
...
```bash
# 快速冒烟测试
drun run testcases -env dev -k "smoke" -failfast
```
...
# 2. 显示敏感信息
drun run test_api -env dev -secrets plain

# 3. 查看响应头
drun run test_api -env dev -response-headers

# 4. 组合使用
drun run test_api -env dev -log-level DEBUG -secrets plain -response-headers
```
