...
```bash
# 启用调试日志
drun run test_api --env dev --log-level DEBUG --reveal-secrets
```
...
**解决**：使用 `--mask-secrets`
```bash
drun run testcases --env dev --html report.html --mask-secrets
```
...
# 2. 显示敏感信息
drun run test_api --env dev --reveal-secrets

# 3. 查看响应头
drun run test_api --env dev --response-headers

# 4. 组合使用
drun run test_api --env dev --log-level DEBUG --reveal-secrets --response-headers
```
