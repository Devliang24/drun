...
| `invoke_case_name` | string | 仅执行 invoke 文件中匹配该 `config.name` 的 case |
| `invoke_case_names` | list[string] | 执行 invoke 文件中多个匹配的 case（去重后生效） |
| `extract` | dict | 响应数据提取 |
| `validate` | list | 断言列表 |
...
| `repeat` | int/string | 步骤执行次数（支持表达式，如 `${retry_count}`） |
| `retry` | int | 重试次数 |
| `retry_backoff` | float | 重试间隔（指数退避） |

## 插件机制（Hooks）

Drun 通过 `Dhook.py` 文件实现自定义函数扩展。

### 文件位置

插件文件必须命名为 `Dhook.py`，放在项目根目录：
