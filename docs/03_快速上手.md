...
如果你只是想快速测试一个 API，可以使用 `drun quick`（简写 `drun q`），无需创建 YAML 文件：

```bash
# 快速 GET 请求
$ drun q https://httpbin.org/get?foo=bar

# 快速 POST 请求
$ drun q https://httpbin.org/post -X POST -d '{"name": "test"}'

# 带断言和提取
$ drun q https://httpbin.org/get --validate 'eq status_code 200' --extract 'origin=$.origin'
```
...
```bash
# 运行单个测试
$ drun run test_demo --env dev

# 运行并生成 HTML 报告
$ drun run test_demo --env dev --html reports/demo.html
...
```bash
$ drun run test_users --env dev --html reports/users.html
```
