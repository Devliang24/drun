...
```bash
# 自定义 HTML 报告路径
drun run testcases -env dev -html custom/path/report.html

# 自定义 JSON 报告路径
drun run testcases -env dev -report custom/path/results.json
```
...
test:
  script:
    - drun run testcases -env ci -html reports/report.html
  artifacts:
    when: always
    paths:
      - reports/
