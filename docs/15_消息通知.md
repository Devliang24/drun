...
# 仅通过时通知
drun run testcases -env dev -notify feishu -notify-only passed

# 始终通知
drun run testcases -env dev -notify feishu -notify-only always
```
...
```bash
# 仅失败时通知（推荐 CI/CD 使用）
drun run testcases -env ci -notify feishu -notify-only failed
```
...
```bash
# 启用 DEBUG 日志查看详情
drun run testcases -env dev -notify feishu -log-level DEBUG
```
