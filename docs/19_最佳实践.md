...
# CI/CD 环境启用脱敏
drun run testcases -env ci -secrets mask
```
...
# 显示敏感信息
drun run test_order -env dev -secrets plain

# 完整调试
drun run test_order -env dev -log-level DEBUG -secrets plain -response-headers
```
    ...
    steps:
      ...
      - name: Install & Run
        run: |
          ...
          drun run testsuite_smoke -env ci -html reports/report.html -secrets mask
...
```bash
# 禁用生成
drun run testcases -env dev -snippet off
