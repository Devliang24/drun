...
|------|------|
| `drun init <name>` | 创建项目 |
| `drun run <path> --env <env>` | 运行测试 |
| `drun run testcases -k "smoke"` | 标签过滤 |
| `drun run testcases --html report.html` | 生成报告 |
...
# 用例 B 直接使用
drun run test_create_order --env dev
# 自动读取 TOKEN
```
...
# CI/CD 环境启用脱敏
drun run testcases --env ci --mask-secrets
```
...
```bash
# 正常运行
drun run test_order --env dev

# 调试模式
drun run test_order --env dev --log-level DEBUG

# 显示敏感信息
drun run test_order --env dev --reveal-secrets

# 完整调试
drun run test_order --env dev --log-level DEBUG --reveal-secrets --response-headers
```
    ...
    steps:
      ...
      - name: Install & Run
        run: |
          ...
          drun run testsuite_smoke --env ci --html reports/report.html --mask-secrets
...
```bash
# 禁用生成
drun run testcases --env dev --no-snippet

# 仅生成 cURL
drun run testcases --env dev --snippet-lang curl

# 仅生成 Python
drun run testcases --env dev --snippet-lang python

# 自定义输出目录
drun run testcases --env dev --snippet-output exports/
```
