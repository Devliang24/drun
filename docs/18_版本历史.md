...
> 说明：本页中的评分系统相关内容仅作为历史版本记录保留，不代表当前版本仍提供 `drun score` 命令。

## v7.1.0 (2025-12-29)

### 快速调试模式

- **新增**: `drun quick` / `drun q` 命令
  - 无需 YAML 和环境文件，直接执行 HTTP 请求
  - httpie 风格的命令行参数：`-X`、`-H`、`-d`、`-p` 等
  - 支持断言（`-validate`）和变量提取（`-extract`）
  - 默认输出 Status + 截断 Body，支持 `-v` 详细模式
  - 支持保存响应体（`-o`）和转存为 YAML 用例（`-save-yaml`）
  - 跨平台支持，Windows 可使用 `-data-file` 传递请求体
  - 退出码：0=成功，1=断言失败，2=参数/请求错误
...
# 转存为 YAML 用例
drun q https://api.example.com/users -save-yaml testcases/quick.yaml
```
