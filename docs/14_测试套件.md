...
### invoke 文件内按 case name 选择执行

默认情况下，`invoke` 会按兼容行为执行被调用文件中的默认 case。  
如果需要在被调用文件中精确挑选某个或多个 case，可使用以下字段：

```yaml
caseflow:
  - name: 渠道 Token 流程测试（单选）
    invoke: test_channel_token_flow
    invoke_case_name: 获取渠道 token

  - name: 渠道 Token 流程测试（多选）
    invoke: test_channel_token_flow
    invoke_case_names:
      - 获取渠道 token
      - 刷新渠道 token
...
- `invoke_case_name` 与 `invoke_case_names` 互斥，不能同时配置
- `invoke_case_names` 会自动去重
- 多选执行顺序按被调用文件中的 case 原始顺序，而不是列表书写顺序
- 如果目标名称在被调用文件中重复出现，会全部命中并按文件顺序执行
- 如果没有命中任何 case，当前 invoke 步骤会失败，并提示可用 case 名称

### caseflow 步骤循环（repeat）

`caseflow` 的 invoke 步骤同样支持 `repeat`：
...
- `repeat` 支持整数或表达式（如 `${retry_count}`）
- 重复执行时，日志/报告会在步骤名后追加 `[repeat=i/N]`
...
# 运行套件目录下所有套件
drun run testsuites -env dev

# 带报告
drun run testsuite_e2e -env dev -html reports/e2e.html

# 标签过滤
drun run testsuites -env dev -k "smoke"
```
