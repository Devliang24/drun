...
| 字段 | 类型 | 说明 |
|------|------|------|
| `save_body_to` | string | 将原始响应字节保存到指定路径 |
...
也支持单字符串路径写法，默认按文件扩展名推断 `Content-Type`，推断失败时使用 `application/octet-stream`：
...
- `files.<field>: "path/to/file"`：本地文件路径
- `files.<field>: ["path/to/file", "mime/type"]`：本地文件路径 + 显式内容类型
- `data + files` 会按 multipart/form-data 发送
- `body + files` 不支持混用；multipart 表单字段请写在 `data` 中
