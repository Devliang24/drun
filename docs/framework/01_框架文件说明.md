...
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  cli.py                                                              │   │
│  │  - drun run test_api -env dev    # 运行测试                           │   │
...
│   └── report.py       # RunReport/CaseInstanceResult/StepResult 报告模型
│
...
│   ├── builtins.py     # 内置函数 (uuid, now, random_int, fake_* 等 18 个)
...
| 命令 | 功能 | 示例 |
|------|------|------|
| `drun run` / `drun r` | 运行测试 | `drun run test_login -env dev` |
...
| `drun server` / `drun s` | 报告服务器 | `drun server -port 8080` |
...
| `report.py` | `RunReport`, `CaseInstanceResult`, `StepResult` | 测试报告数据结构 |
...
| `Dhook.py` | `find_hooks()`, `get_functions_for()` | 钩子函数加载 |
...
| `builtins.py` | `BUILTINS` 字典 | 18 个内置函数 |
...
│                                                                 │
│  6. 项目脚手架 (scaffolds/)   - 如不需要 drun init 命令          │
