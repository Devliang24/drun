...
def _build_case(case: CaseInstanceResult, case_idx: Optional[int] = None) -> str:
    ...
    head = (
        "<div class='head'>"
        f"<div><div><b>{case_prefix}</b>{_escape_html(case.name)}{meta_html}</div>{params_html}</div>"
    ...
    for idx, s in enumerate(case.steps or []):
        steps_html_parts.append(_build_step(s, step_idx=idx + 1 if total_steps > 1 else None))
...
"""
    ...
    for idx, c in enumerate(merged_cases):
        body_cases.append(_build_case(c, case_idx=idx + 1 if len(merged_cases) > 1 else None))
