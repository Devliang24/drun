...
class SnippetGenerator:
    ...
    def generate_python_script_for_step(
        ...
        lines.append(f'    print("=== Step {step_idx + 1}: {step.name} ===")')
        lines.append("")
        ...
        if dependencies:
            ...
            lines.append(f"    if not {dependencies[0].lower()}:")
            lines.append(f'        print("⚠️  Warning: {dependencies[0]} not set", file=sys.stderr)')
            lines.append("")
        
        # 生成请求代码
        step_code = self._generate_python_request(case, step_idx, envmap)
        lines.extend(f"    {line}" for line in step_code.split('\n') if line)
        
        lines.append("")
        lines.append('    print(f"Status: {response.status_code}")')
        lines.append('    try:')
        lines.append('        print(json.dumps(response.json(), indent=2, ensure_ascii=False))')
