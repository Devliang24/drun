...
def execute_invoke_step(
    *,
    ...
    if isinstance(invoked_base_url, str) and ("${" in invoked_base_url or "{{" in invoked_base_url):
        try:
            invoked_case.config.base_url = runner._render(invoked_base_url, invoke_vars, funcs, envmap)
        except Exception as exc:
            if runner.log:
                runner.log.error(f"[INVOKE] Failed to render base_url '{invoked_base_url}': {exc}")
            raise

    if runner.log:
        runner.log.info(f"[INVOKE] Executing: {invoked_case.config.name or resolved_path.name}")

    param_sets = expand_parameters(invoked_case.parameters, source_path=str(resolved_path))
    if runner.log and len(param_sets) > 1:
        runner.log.info(f"[INVOKE] Expanding {len(param_sets)} parameter sets")
