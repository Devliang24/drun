...
**Request Step（请求步骤）**:
Executable Target 为 `request` 的 Step，用于发送 HTTP 请求，并可从响应中 validate 或 extract。
...
**Sleep Step（等待步骤）**:
Executable Target 为 `sleep` 的 Step，用于按配置的毫秒数等待，且不能携带 validate 或 extract 等 request-only 字段。
_Avoid_: delay action, 延迟动作

**StepResult（步骤结果）**:
一个已执行 Step 的运行期结果记录，包含状态、耗时、断言、请求与响应信息、错误信息等。
_Avoid_: step output, 步骤输出

**Step Outcome（步骤产出）**:
Request Step 在收到响应后形成 StepResult 之前的响应后处理结果，包含 validate、extract、persist、export、response.save_body_to 和 StepResult 所需字段。
...
Dev: “Request Step 的 validate、extract、persist、export 和 response.save_body_to 都算 Step Outcome 吗？”
